...
if(!isNull (_obj getVariable ["jammer_zone", objNull])) exitWith {hint "Jammer cannot be created on top of another jammer!"};
...
[_trg, ["this && thisTrigger getVariable ['jammer_active',false] && alive (thisTrigger getVariable ['jammer_object',objNull]) && acre_player in thisList", "[thisTrigger, acre_player, true] call twc_acre_cussig_fnc_addJammerToPlayer", "[thisTrigger, acre_player, false] call twc_acre_cussig_fnc_addJammerToPlayer"]] remoteExec ["setTriggerStatements", 0, _trg];
